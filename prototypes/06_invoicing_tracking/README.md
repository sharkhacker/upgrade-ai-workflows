...
1. Scans `data/invoices.csv`:
   ...
   - Overdue → picks the dunning stage (1–14 days gentle / 15–30 firm / 30+ final notice) and drafts the stage-appropriate email via `shared/prompts/dunning_sequence.md`.
   - `paid` / current → logged, no action.
2. Writes drafts to `outputs/invoicing/` and an action log to `outputs/invoicing_actions.csv`.
