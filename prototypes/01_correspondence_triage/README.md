...
2. Classifies each item with Claude/GPT using the prompt in `shared/prompts/correspondence_classifier.md` — category, urgency, deadline extraction, notify-or-not decision.
3. Appends every item to `outputs/correspondence_log.csv` (the digital mailroom log).
4. For items needing client attention, drafts **three channel versions** of the notification (email / WhatsApp / WeChat, Chinese where the client prefers it) using `shared/prompts/client_notification.md`.
...
No notification is auto-sent. Drafts land in `outputs/notifications/` for a staff member to review — urgent legal items are flagged `!!` for same-day handling.
